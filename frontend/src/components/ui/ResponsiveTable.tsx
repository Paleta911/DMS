...
export function ResponsiveTable<T>({
  ...
  const getColumnLabel = useCallback(
    (column: ResponsiveColumn<T>, index: number) => column.header.trim() || `Columna ${index + 1}`,
    [],
  );
  const columnIds = useMemo(
    () => columns.map((column, index) => column.id ?? `${getColumnLabel(column, index)}-${index}`),
  ...
  const getMeasuredWidths = useCallback(() => {
    return columns.map((column, index) => {
      const measuredWidth = Math.round(headerRefs.current[index]?.getBoundingClientRect().width ?? 0);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      nextWidths[dragState.columnIndex] = Math.max(
        minWidth,
        dragState.startWidths[dragState.columnIndex] + (event.clientX - dragState.startX),
      );
  ...
  const startResize = (columnIndex: number, clientX: number) => {
    const baseWidths = columnWidths.length === columns.length ? columnWidths : getMeasuredWidths();
  ...
  const handleResizeMouseDown = (columnIndex: number, event: ReactMouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
    event.stopPropagation();
    startResize(columnIndex, event.clientX);
  };

  const handleResizeKeyDown = (columnIndex: number, event: ReactKeyboardEvent<HTMLButtonElement>) => {
    ...
    nextWidths[columnIndex] = Math.max(minWidth, baseWidths[columnIndex] + delta);
    setColumnWidths(nextWidths);
  };
  ...
  const tableWidth = useMemo(() => {
    ...
    const totalWidth = columns.reduce((sum, column, index) => {
      ...
      return sum + (columnWidths[index] ?? column.width ?? column.minWidth ?? 140);
    }, 0);
        ...
        <div className="flex justify-end" ref={columnMenuRef}>
          <div className="relative">
            <button
              ...
              aria-label={isColumnMenuOpen ? 'Ocultar menú de columnas' : 'Mostrar menú de columnas'}
              ...
              <div className="absolute right-0 z-20 mt-2 min-w-60 rounded-xl border border-brand-border bg-brand-surface p-2 shadow-soft">
                ...
                <div className="grid gap-1">
                  {columns.map((column, index) => {
                    ...
                    const canHide = (column.hideable ?? true) && visibleColumnCount > 1;
                    return (
                      <button
                        key={columnId}
                        type="button"
                        ...
                        disabled={!(column.hideable ?? true) || (!canHide && isVisible)}
                      >
                        <span className="truncate">{getColumnLabel(column, index)}</span>
                        {isVisible ? <Eye size={16} /> : <EyeOff size={16} className="text-brand-textMuted" />}
                      </button>
                    );
                  })}
                </div>
        ...
        >
          ...
          >
                ...
                return (
                  <col
                    ...
                    style={{
                      ...
                      minWidth:
                        typeof column.minWidth === 'number' ? `${column.minWidth}px` : undefined,
                    }}
                  />
                );
              })}
                  ...
                  >
                    <div className="relative flex min-h-6 items-center pr-4">
                      ...
                      <button
                        ...
                        onMouseDown={(event) => handleResizeMouseDown(index, event)}
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td colSpan={visibleColumns.length} className="px-4 py-6 text-brand-textMuted">
                    {emptyLabel}
                  </td>
                </tr>
              ) : (
                <>
                  {shouldVirtualize && visibleRange.topSpacer > 0 ? (
                    <tr aria-hidden="true">
                      <td colSpan={visibleColumns.length} style={{ height: `${visibleRange.topSpacer}px` }} />
                    </tr>
                  ) : null}
                  ...
                  <motion.tr
                    ...
                    className={[
                      ...
                      onRowClick ? 'cursor-pointer transition hover:bg-brand-bg/60' : '',
                  ...
                  {shouldVirtualize && visibleRange.bottomSpacer > 0 ? (
                    <tr aria-hidden="true">
                      <td colSpan={visibleColumns.length} style={{ height: `${visibleRange.bottomSpacer}px` }} />
                    </tr>
                  ) : null}
                </>
              )}
