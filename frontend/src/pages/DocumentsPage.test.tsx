...
// Page tests cover permissions, filters, admin visibility toggles, and saved filter restoration.
...
describe("DocumentsPage", () => {
  beforeEach(() => {
    ...
    mocks.api.documentTypesList.mockResolvedValue([
      { id: 1, code: "MAN", nombreLargo: "Manual", activo: true },
    ]);
    mocks.api.areaCodesList.mockResolvedValue([
  ...
  it("permite ver documentos aunque el usuario no tenga áreas asignadas", async () => {
    ...
    expect(
      (await screen.findAllByText("Manual de calidad")).length,
    ).toBeGreaterThan(0);
  ...
  it("activa el modo de búsqueda avanzada y dispara la consulta filtrada", async () => {
    ...
    expect(
      await screen.findByText("Búsqueda avanzada activa"),
    ).toBeInTheDocument();
  });
  ...
  it("muestra controles globales de visibilidad para admins", async () => {
    ...
    expect(
      await screen.findByText("Visibilidad global de documentos"),
    ).toBeInTheDocument();
  ...
  it("restaura los filtros del usuario al salir y volver a la ruta", async () => {
    ...
    expect((screen.getByLabelText("Límite") as HTMLSelectElement).value).toBe(
      "10",
    );
  });
});
