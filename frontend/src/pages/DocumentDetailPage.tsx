...
} from "../components/users/UserSearchInput";
import { NoticeBanner } from "../components/ui/NoticeBanner";
import { translateStatus } from "../utils/labels";
import { getApiErrorMessage } from "../utils/apiError";
import { queryClient } from "../app/queryClient";
import {
  invalidateDocumentScopeQueries,
...
function getTextExtractionBadge(version: Version) {
  if (version.textSource === "PDF_OCR") {
    return {
      tone: "WARN" as const,
      label: version.ocrPageCount
        ? `OCR (${version.ocrPageCount} pág.)`
        : "OCR",
...
export default function DocumentDetailPage() {
  ...
  const [confirmOpen, setConfirmOpen] = useState<null | "submit" | "obsolete">(
    null,
  );
  ...
  const reviewer = useMemo(
    () =>
      approvals.find((approval) => approval.step === "REVISO")?.user?.email ??
      "Sin asignar",
    [approvals],
  );
  const approver = useMemo(
    () =>
      approvals.find((approval) => approval.step === "APROBO")?.user?.email ??
      "Sin asignar",
    [approvals],
  );
  const elaborador = useMemo(
    () =>
      approvals.find((approval) => approval.step === "ELABORO")?.user?.email ??
      "Sin asignar",
    [approvals],
  );
  ...
  const uploadIsInternal = Boolean(documentQuery.data?.isInternal);
  const canUploadVersion = () => {
    if (isAdmin) return true;
    return Boolean(user?.permissions?.canUploadNewVersion);
  };

  const revisoEligibility = getAssignmentEligibility(
    assignUsers.reviso,
    "review",
  );
  const aproboEligibility = getAssignmentEligibility(
    assignUsers.aprobo,
    "approve",
  );
  ...
  const submitUpload = async () => {
    if (!canUploadVersion()) {
      notify("No tienes permiso para subir nuevas versiones", "error");
      return;
    }
    if (!uploadForm.file) {
    ...
    if (uploadIsInternal && !uploadForm.documentTypeCode) {
      notify("Tipo requerido para documentos internos", "error");
      return;
    }
    if (uploadIsInternal && !uploadForm.areaCode) {
  ...
  const submitAssign = async () => {
    ...
    try {
      await workflowAssign(
        documentId,
        validation.data.revisoUserId,
        validation.data.aproboUserId,
      );
  ...
  const buildDownloadName = (
    ...
    const code = documentQuery.data?.codigo?.trim();
    ...
    return originalName ?? storedName ?? `version-${versionId}`;
  };
  ...
  const handleDownload = async (
    ...
    try {
      ...
      downloadBlob(
        blob,
        buildDownloadName(versionId, originalName, storedName),
      );
    } catch (error: any) {
    ...
    { header: "Fecha", cell: (version) => formatDate(version.createdAt) },
    {
      ...
      cell: (version) => (
        <Button
          variant="outline"
          onClick={() =>
            handleDownload(version.id, version.originalName, version.storedName)
          }
        >
          Descargar
        </Button>
      ...
      <section className="flex flex-col gap-6">
            ...
            actions={
              <ResponsiveActions>
                <Button
                  variant="outline"
                  onClick={openUpload}
                  disabled={actionLoading !== null || !canUploadVersion()}
                >
                  Subir versión
                </Button>
                {isAdmin ? (
        ...
        {loading ? (
          ...
          <FadeInSection delay={0.05}>
            ...
            >
              Vuelve al listado y abre de nuevo el documento. Si el problema
              persiste, verifica tus permisos.
            </NoticeBanner>
          </FadeInSection>
        ) : (
          <>
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="font-display text-2xl text-ink">
                      {documentQuery.data?.nombre}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2">
                      <StatusBadge
                        status={documentQuery.data?.status ?? "DRAFT"}
                      />
                      {documentQuery.data?.documentType?.code ? (
                        <Pill>{documentQuery.data.documentType.code}</Pill>
                      ) : null}
                      {documentQuery.data?.areaCode?.code ? (
                        <Pill>{documentQuery.data.areaCode.code}</Pill>
                      ) : null}
                    </div>
                  </div>
            ...
            <FadeInSection delay={0.1}>
              <NoticeBanner
                ...
                variant={
                  isApprover || isReviewer || isElaborador || isAdmin
                    ? "info"
                    : "success"
                }
              >
                  ...
                  {versions.length === 0 ? (
                    <div className="mt-4">
                      <NoticeBanner title="Sin versiones registradas">
                        Este documento todavía no tiene archivos asociados para
                        descarga.
                      </NoticeBanner>
                    </div>
                  ) : (
        ...
        <Modal
          open={assignOpen}
          title="Asignar revisión / aprobación"
          onClose={() => setAssignOpen(false)}
        >
          <div className="grid gap-4">
            <NoticeBanner title="Selecciona responsables aptos">
              El usuario de revisión debe tener permiso para revisar y el de
              aprobación debe tener permiso para aprobar.
            </NoticeBanner>
            <UserSearchInput
              ...
              onSelect={(user) =>
                setAssignUsers((prev) => ({ ...prev, reviso: user }))
              }
            />
            <UserSearchInput
              ...
              onSelect={(user) =>
                setAssignUsers((prev) => ({ ...prev, aprobo: user }))
              }
            />
            <ResponsiveActions>
              ...
              <Button
                ...
                disabled={actionLoading === "assign" || !canSaveAssignment}
              >
        ...
        <Modal
          ...
          title={
            confirmOpen === "submit" ? "Enviar a revisión" : "Marcar obsoleto"
          }
          onClose={() => setConfirmOpen(null)}
        >
          <div className="grid gap-4">
            <NoticeBanner
              variant={confirmOpen === "obsolete" ? "warning" : "info"}
            >
              {confirmOpen === "submit"
        ...
        >
          <div className="grid gap-4">
            <NoticeBanner title="Mantén la trazabilidad del documento">
              La nueva versión conservará si el documento es interno y el mismo
              código actual. Si el archivo es un PDF escaneado, el sistema
              intentará aplicar OCR para volverlo buscable por contenido dentro
              del sistema.
            </NoticeBanner>
            <Input
            ...
            <ResponsiveActions>
              ...
              <Button
                ...
                disabled={actionLoading === "upload" || !canUploadVersion()}
              >
