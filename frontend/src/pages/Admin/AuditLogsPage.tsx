...
function normalizeMeta(meta: unknown): Record<string, unknown> {
  ...
  if (typeof meta === 'string') {
    try {
      ...
      return typeof parsed === 'object' && parsed ? (parsed as Record<string, unknown>) : { valor: parsed };
    } catch {
      return { valor: meta };
    }
  }
...
function filtersToSearchParams(filters: Filters) {
  ...
  if (filters.page !== INITIAL_FILTERS.page) params.set('page', String(filters.page));
  if (filters.limit !== INITIAL_FILTERS.limit) params.set('limit', String(filters.limit));
  return params;
}
...
export default function AuditLogsPage() {
  ...
  const columns: ResponsiveColumn<AuditLog>[] = [
      ...
      cell: (log) => (
        <button
          ...
          onClick={() => {
            ...
            setAppliedFilters((prev) => ({ ...prev, user: userValue, page: 1 }));
          }}
        >
          {log.userId ?? '-'}
        </button>
    ...
    { header: 'Tipo', cell: (log) => formatResourceTypeLabel(log.resourceType) },
    { header: 'Recurso', cell: (log) => log.resourceId ?? '-' },
    { header: 'IP', cell: (log) => log.ip ?? '-' },
    {
      header: '',
      cell: (log) => (
        <Button variant="outline" onClick={() => setSelectedMeta(normalizeMeta(log.meta))}>
          Detalle
        </Button>
      ),
    },
  ...
  return (
    <PageContainer>
      <section className="flex flex-col gap-6">
        <PageHeader title="Auditoría" subtitle="Bitácora de eventos y acciones del sistema." />
            ...
            toolbar={{
              summary: (
                <>
                  ...
                  {activeFilterCount > 0 ? <span>{activeFilterCount} filtro(s) activo(s)</span> : null}
                </>
              ),
              currentPage: page,
              totalPages,
              ...
              onNext: () => setAppliedFilters((prev) => ({ ...prev, page: prev.page + 1 })),
            ...
            <div className="flex flex-col gap-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-sm text-brand-textMuted">{formatDate(log.createdAt)}</div>
                  </div>
                  <span className="text-xs text-brand-textMuted">Usuario: {log.userId ?? '-'}</span>
                </div>
                <div className="grid gap-2 text-xs text-brand-textMuted sm:grid-cols-2">
                  ...
                  <div>
                    <span className="text-brand-textMuted">Recurso:</span> {log.resourceId ?? '-'}
                  </div>
                  <div>
                    <span className="text-brand-textMuted">IP:</span> {log.ip ?? '-'}
                  </div>
                </div>
                <ResponsiveActions>
                  <Button
