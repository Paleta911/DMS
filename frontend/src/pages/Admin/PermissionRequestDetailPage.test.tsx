...
describe('PermissionRequestDetailPage', () => {
  ...
  it('permite aprobar parcialmente una solicitud de áreas', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nota de aprobación parcial (opcional)'), {
    ...
    await waitFor(() => {
      expect(mocks.api.adminPermissionRequestApprovePartial).toHaveBeenCalledWith(15, {
        areaCodes: ['RC'],
        note: 'Se autoriza solo el área principal',
      });
      expect(mocks.notify).toHaveBeenCalledWith('Aprobación parcial aplicada', 'success');
    });
  });
