...
import type { PermissionRequest, PermissionRequestStatus, PermissionRequestType } from '../../types/permissions';
...
const statusOptions: Array<{ value: PermissionRequestStatus | 'ALL'; label: string }> = [
...
const typeOptions: Array<{ value: 'ALL' | 'PERMISSIONS' | 'AREAS'; label: string }> = [
  { value: 'ALL', label: 'Todos' },
...
export default function PermissionRequestsPage() {
  ...
  const [status, setStatus] = useState<PermissionRequestStatus | 'ALL'>(initialFilters.status);
  const [typeFilter, setTypeFilter] = useState<'ALL' | 'PERMISSIONS' | 'AREAS'>(initialFilters.typeFilter);
  ...
  }, [detailFilter, limit, page, rememberLastUsed, status, typeFilter, userFilter]);
  ...
  const requestsQuery = useQuery({
    ...
    queryFn: () =>
      adminPermissionRequestsList({
        ...
        type: typeFilter === 'ALL' ? undefined : (typeFilter as PermissionRequestType),
  ...
  const exportFiltered = async () => {
    ...
    try {
      ...
      downloadBlob(blob, `solicitudes_filtradas_${new Date().toISOString().slice(0, 10)}.csv`);
    } catch (error: any) {
  ...
  return (
    <PageContainer>
      <section className="flex flex-col gap-6">
        <PageHeader title="Solicitudes" subtitle="Aprueba solicitudes de permisos y áreas." />
        <FilterCard
        ...
        >
            ...
            <Select
              ...
              value={typeFilter}
              onChange={(event) => {
                setTypeFilter(event.target.value as 'ALL' | 'PERMISSIONS' | 'AREAS');
                setPage(1);
              }}
            >
        ...
        <DataTableSection
          toolbar={{
            summary: (
              <>
                <span>
                  Mostrando {requestsQuery.data?.items?.length ?? 0} de {requestsQuery.data?.total ?? 0}
                </span>
                <span>Página {requestsQuery.data?.page ?? page}</span>
              </>
            ),
            currentPage: requestsQuery.data?.page ?? page,
            totalPages: Math.max(1, Math.ceil((requestsQuery.data?.total ?? 0) / limit)),
          ...
          renderMobileCard={(item) => (
            <div className="flex flex-col gap-3">
              <div className="font-semibold text-brand-text">{item.user?.email ?? '-'}</div>
              ...
              <div className="text-sm text-brand-textMuted">
                Detalle:{' '}
                {getPermissionRequestDetail(item)}
              </div>
              <AdminActionList
                actions={buildPermissionRequestActions(item, {
                  onApprove: (id) => approveMutation.mutate(id),
