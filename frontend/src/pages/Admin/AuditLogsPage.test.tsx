...
// Audit logs page tests cover admin gating, detail modal, exports, URL-state, and persisted filters.
...
describe("AuditLogsPage", () => {
  beforeEach(() => {
    ...
    mocks.api.auditLogsExportCsv.mockResolvedValue(
  ...
  it("exporta el CSV filtrado", async () => {
    ...
    fireEvent.click(
      screen.getByRole("menuitem", { name: "Exportar CSV filtrado" }),
    );
  ...
  it("recupera filtros guardados y los vuelve a guardar al aplicar cambios", async () => {
    ...
    await waitFor(() => {
      expect(
        JSON.parse(
          window.localStorage.getItem("audit-log-filters:admin@local.com") ??
            "{}",
        ),
      ).toMatchObject({
        action: "ACCESS_DENIED",
