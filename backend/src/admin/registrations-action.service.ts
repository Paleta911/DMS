...
export class RegistrationsActionService {
  ...
  async approve(params: {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (user.status !== UserStatus.PendingApproval) {
        throw new BadRequestException('Registro no está pendiente de aprobación');
      }
      ...
      user.canUploadNewVersion = DEFAULT_APPROVED_PERMISSIONS.canUploadNewVersion;
  ...
  async resendCode(params: {
    ...
    if (user.status !== UserStatus.PendingVerification) {
      throw new BadRequestException('Registro no está pendiente de verificación');
    }
