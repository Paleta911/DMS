...
// Ensures document type mutations enforce uniqueness and clear document references on deactivate.
describe('DocumentTypesMutationService', () => {
  ...
  it('desactiva el tipo y limpia documentos relacionados', async () => {
    ...
    documentTypeRepo.findOne.mockResolvedValue({
      id: 9,
      code: 'PRO',
      nombreLargo: 'Procedimiento',
      activo: true,
    ...
    expect(updateChain.where).toHaveBeenCalledWith('documentTypeId = :id', {
      id: 9,
    });
